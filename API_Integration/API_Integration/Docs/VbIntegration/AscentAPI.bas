...
' Returns JSON array of active academic years (status=Active only).
' Fields per item: academicYearId, academicYear, startMonth, endMonth, isCurrent
' isCurrent=true marks the most recently created Active year — use this as default.
Public Function GetAcademicYears() As String
    GetAcademicYears = JsonData(ApiGet("school/master/academic-years?activeOnly=true"))
End Function
...
' Get list of receipts. Pass "" to skip any filter.
' sDateFrom / sDateTo : "YYYY-MM-DD"
' sCreatedAfter       : ISO 8601 datetime e.g. "2025-06-01T18:00:00" — for EOD sync
' sSource             : "webapp" to get only web/portal receipts (avoids sync loop);
'                       "legacy" to get only receipts pushed from VB6; "" for all
' Returns JSON array. Fields per item:
'   receiptId, receiptNo, studentName, admissionNo,
'   totalAmount, paymentModeName, paymentDate, status
Public Function GetReceipts(Optional sDateFrom As String = "", _
                            Optional sDateTo As String = "", _
                            Optional sSearch As String = "", _
                            Optional sCreatedAfter As String = "", _
                            Optional sSource As String = "") As String
    Dim sQuery As String
    ...
    If Len(sSearch)       > 0 Then sQuery = sQuery & "search="       & sSearch       & "&"
    If Len(sCreatedAfter) > 0 Then sQuery = sQuery & "createdAfter=" & sCreatedAfter & "&"
    If Len(sSource)       > 0 Then sQuery = sQuery & "source="       & sSource       & "&"
...
'==============================================================================
' SECTION 9 — RECEIPT SYNC (historical import + EOD push from legacy)
' Use BulkImportReceipts for both one-off historical import and daily EOD sync.
' Max 1000 rows per call. Re-running is safe — duplicate LegacyReceiptNo is skipped.
'==============================================================================

' Bulk import / sync fee receipts from legacy SAS to the new app.
'
' Each row in sJsonArray:
'   LegacyReceiptNo : FeeReciptID from SAS_FeeReceipts — used as receipt_no directly.
'                     Duplicate check: if receipt_no already exists it is skipped
'                     (safe to re-run the same batch).
'   AdmissionNo     : StuAdmnNo
'   AcademicYear    : AcdYear  e.g. "2024-25"
'   FeeCategory     : "School" for standard fees (Transport / Hostel / Other also valid)
'   FeeTypeName     : FeeTyp
'   TermName        : PaymentTyp (term name)
'   PaidAmount      : FeeAmt
'   PaymentDate     : FeeDat  "YYYY-MM-DD"
'   PaymentMode     : PymntTyp e.g. "Cash"
'   ChequeNo        : RefNo  (optional)
'   Remarks         : RemarksDet  (optional)
'   Status          : "A" → Active;  "D" → Cancelled  (default Active if blank)
'   CancelReason    : DeletResn  (required when Status="D")
'
' Returns result JSON: {total, imported, skipped, failed, errors:[{row, admissionNo, reason}]}
' Use BuildReceiptRow() to construct each row object.
...
' Build one receipt row object for BulkImportReceipts.
' sReceiptId    : FeeReciptID from SAS_FeeReceipts
' sStatus       : "A" (Active) or "D" (Cancelled) — maps from FeeReceiptStat
' sCancelReason : DeletResn from SAS_FeeReceipts when sStatus = "D"
Public Function BuildReceiptRow(sReceiptId As String, _
                                sAdmNo As String, _
                                sAcadYear As String, _
                                sFeeTypeName As String, _
                                sTermName As String, _
                                dPaidAmount As Double, _
                                sPaymentDate As String, _
                                sPaymentMode As String, _
                                Optional sChequeNo As String = "", _
                                Optional sRemarks As String = "", _
                                Optional sStatus As String = "A", _
                                Optional sCancelReason As String = "") As String
    BuildReceiptRow = BuildObj( _
        "LegacyReceiptNo", sReceiptId, _
        "AdmissionNo",     sAdmNo, _
        "AcademicYear",    sAcadYear, _
        "FeeCategory",     "School", _
        "FeeTypeName",     sFeeTypeName, _
        "TermName",        sTermName, _
        "PaidAmount",      dPaidAmount, _
        "PaymentDate",     sPaymentDate, _
        "PaymentMode",     sPaymentMode, _
        "ChequeNo",        sChequeNo, _
        "Remarks",         sRemarks, _
        "Status",          sStatus, _
        "CancelReason",    sCancelReason _
    )
End Function
