...
├── Models/Legacy/                   ← POCO classes matching VB6 DB columns (LegacyAcademicYear, LegacyClass, LegacyClassGroup, LegacyFeeCategory, LegacyFeeMaster, LegacyFeeType, LegacyTerm, LegacyBusRoute, LegacyBusMaster, LegacyStudent, LegacyBus, LegacyPaymentMode, LegacyFeeReceipt)
└── Migrators/
    ...
    ├── StudentsMigrator.cs           ← SAS_StudentMaster → students (depends on academic_years, classes, sections, bus_routes); 5 lookup maps; ~50 mapped columns; StuFamilyChildres varchar→int TryParse; StuUnqID float→int; status A→Active/D→Inactive/R→R; is_detained=0; TakeFirstMobile splits StuMobile1/2 on '/',',' caps at 10 digits for father_mobile/mother_mobile; full raw stored in spare_field_1/2; skip key: admission_no|academic_year_id
